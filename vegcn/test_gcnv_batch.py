...
from vegcn.dataset.gcn_v_dataset import GCNVDatasetBatch
...
def test_gcnv_batch(cfg, batch_num):
    torch.set_grad_enabled(False)
    device = cfg["device"]
    # dataset
    cfg["phase"] = "test"
    dataset = GCNVDatasetBatch(cfg, batch_num)
    ...
    model.eval()
    # test data
    features = torch.tensor(dataset.features, dtype=torch.float32).to(device)  # TODO feature in batch form([batch, n//batch, 512])
